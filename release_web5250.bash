...
#          VERSION=1.5 ./release_web5250.bash
#
# Requirements:
...
if gh release view "$TAG" ${REPO_ARGS[@]+"${REPO_ARGS[@]}"} >/dev/null 2>&1; then
    ...
    gh release upload "$TAG" "${ASSETS[@]}" --clobber ${REPO_ARGS[@]+"${REPO_ARGS[@]}"}
else
...
URL="$(gh release view "$TAG" ${REPO_ARGS[@]+"${REPO_ARGS[@]}"} --json url -q .url 2>/dev/null || true)"
